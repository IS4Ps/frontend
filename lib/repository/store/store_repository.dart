...
class StoreRepository {
  ...
  Future<List<ItemModel>?> getStoreItems(int childLevel, int childJobId) async {
    try {
      print("[API 호출] 상점 아이템 조회 시작 (childLevel: $childLevel, childJobId: $childJobId)");

      final response = await _apiService.fetchStoreItems(childLevel, childJobId);
  ...
  Future<bool> purchaseItem(int itemId, int childId) async {
    try {
      print("[API 호출] 아이템 구매 시작 (itemId: $itemId, childId: $childId)");

      final response = await _apiService.purchaseItem(itemId, childId);
